...
def call(metodo, ruta, body=None, auth=True, nota="", token=None):
    ...
    resultados.append((metodo, ruta, codigo, cuerpo, nota))
...
print("-" * 78)
malos = []
for m, ruta, cod, cuerpo, nota in resultados:
    marca = "  " if cod < 400 else ("!!" if cod >= 500 else " ?")
    if cod >= 400:
        malos.append((m, ruta, cod, cuerpo))
    print("%-2s %-7s %-52s %s" % (marca, m, ruta, cod))
...
if malos:
    print("\nDETALLE DE LOS QUE NO DIERON 2xx")
    print("=" * 78)
